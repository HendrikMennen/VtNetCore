...
namespace VtNetCore.XTermParser
{
    public static class XTermSequenceHandlers
    {
                ...
                Handler = (sequence, controller) =>
                {
                    foreach (var parameter in sequence.Parameters)
                        switch (parameter)
                        {
                            ...
                            default:
                                Debug.WriteLine("Set Mode (SM) mode: " + parameter + " is unknown");
                                break;
                        }
                }
            },
                ...
                Handler = (sequence, controller) =>
                {
                    foreach (var parameter in sequence.Parameters)
                        switch (parameter)
                        {
                            ...
                            default:
                                Debug.WriteLine("Reset Mode (RM) mode: " + parameter + " is unknown");
                                break;
                        }
                }
            },
            ...
            new SequenceHandler
            {
                ...
                Handler = (sequence, controller) =>
                {
                    ...
                    else if (csiSequence.Parameters[0] == 38 || csiSequence.Parameters[0] == 48)
                    {
                        ...
                        {
                            Debug.WriteLine("SGR " + csiSequence.Parameters[0] + " must be longer than 1 option");
                        }
                    }
                    else
                    {
        ...
        public static void ProcessSequence(TerminalSequence sequence, IVirtualTerminalController controller)
        {
            ...
            {
                var handler = Handlers.Where(x => x.SequenceType == SequenceHandler.ESequenceType.Character)
                    .SingleOrDefault();
                if (handler == null)
            ...
            {
                var handler = Handlers.Where(x => x.SequenceType == SequenceHandler.ESequenceType.CharacterSet)
                    .SingleOrDefault();
                if (handler == null)
                    throw new EscapeSequenceException(
                        "There are no sequence handlers configured for type CharacterSetSequence " + sequence,
                        sequence);
            ...
            if (sequence is EscapeSequence)
            {
                    ...
                    .Where(x =>
                        ...
                        (
                            ...
                            x.Vt52 == (controller.IsVt52Mode()
                                ? SequenceHandler.Vt52Mode.Yes
                                : SequenceHandler.Vt52Mode.No)
                        )
                    )
                    .SingleOrDefault();
                ...
                if (handler == null)
                    throw new EscapeSequenceException(
                        "There are no sequence handlers configured for type EscapeSequence " + sequence, sequence);
            ...
            {
                var handler = Handlers.Where(x => x.SequenceType == SequenceHandler.ESequenceType.CharacterSize)
                    .SingleOrDefault();
                if (handler == null)
                    throw new EscapeSequenceException(
                        "There are no sequence handlers configured for type CharacterSizeSequence " + sequence,
                        sequence);
            ...
            if (sequence is UnicodeSequence)
            {
                ...
                if (handler == null)
                    throw new EscapeSequenceException(
                        "There are no sequence handlers configured for type UnicodeSequence " + sequence, sequence);
